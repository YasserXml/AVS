    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 p-4">
        ...
        @if (config('media-manager.allow_sub_folders', true))
            @foreach ($folders as $folder)
                <a href="{{ \App\Filament\Resources\SoftwaremediaResource::getUrlFromFolderSoftware($folder) }}"
                    ...
                    <div class="flex flex-col items-center justify-center text-center w-full">
                        ...
                        <div class="flex flex-col items-center gap-1 text-xs text-gray-500 dark:text-gray-400">
                            <span>{{ $folder->subfolders()->count() }} folder, {{ $folder->softwaremedia()->count() }}
                                file</span>
        ...
        @if (isset($records))
            @foreach ($records as $item)
                @if (!($item instanceof \App\Models\Softwarefolder))
