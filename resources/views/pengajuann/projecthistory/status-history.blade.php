    ...
    if (!function_exists('getProjectStatusBadgeColor')) {
        function getProjectStatusBadgeColor($status)
        {
            $colors = [
                ...
                'pending_direksi' => 'bg-yellow-100 text-yellow-800 dark:bg-yellow-800 dark:text-yellow-100',
        ...
        @foreach ($history as $index => $item)
            <div class="relative flex items-start space-x-4 pb-6 {{ $loop->last ? 'pb-0' : '' }}">
                ...
                <div class="relative z-10 flex items-center justify-center">
                    <div
                        class="w-12 h-12 rounded-full flex items-center justify-center {{ $index === 0 ? 'bg-blue-100 dark:bg-blue-800 border-2 border-blue-500' : 'bg-white dark:bg-gray-700 border-2 border-gray-300 dark:border-gray-600' }}">
                ...
                <div class="flex-1 min-w-0">
                    <div
                        class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-600 p-4 {{ $index === 0 ? 'ring-2 ring-blue-500 ring-opacity-50 dark:ring-opacity-50' : '' }}">
                        {{-- Header --}}
                        <div class="flex items-center justify-between mb-3">
                            <div class="flex items-center space-x-2 flex-wrap">
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ getProjectStatusBadgeColor($item['status']) }}">
                        ...
                        @if (isset($item['additional_info']))
                            <div
                                class="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 mb-3 border border-gray-100 dark:border-gray-600">
                                <h4 class="text-xs font-semibold text-gray-700 dark:text-gray-300 mb-2">Informasi
                                    Tambahan:</h4>
                                <p class="text-xs text-gray-800 dark:text-gray-200 leading-relaxed">
                                    {{ $item['additional_info'] }}</p>
                            </div>
                        @endif
                        ...
                        @if (in_array($item['status'], ['ditolak_pm', 'ditolak_pengadaan', 'reject_direksi']) && isset($item['reject_reason']))
                            <div
                                class="bg-red-50 dark:bg-red-900/30 rounded-lg p-3 mb-3 border border-red-200 dark:border-red-800">
                                <div class="flex items-center space-x-2 mb-2">
                                    <x-heroicon-o-exclamation-triangle class="w-4 h-4 text-red-500 dark:text-red-400" />
                                    <h4 class="text-xs font-semibold text-red-700 dark:text-red-300">Alasan Penolakan:
                                    </h4>
                                </div>
                        ...
                        <div
                            ...
                            <div class="flex items-center space-x-2 text-xs text-gray-700 dark:text-gray-200">
                                ...
                                <span
                                    class="text-gray-400 dark:text-gray-400">{{ \Carbon\Carbon::parse($item['created_at'])->setTimezone('Asia/Jakarta')->diffForHumans() }}</span>
                            </div>
                            ...
                            <div class="flex items-center space-x-1">
                                @php
                                    $department = match ($item['status']) {
                                        ...
                                        'disetujui_pm_dikirim_ke_pengadaan', 'ditolak_pm' => [
                                            'PM',
                                            'bg-orange-100 text-orange-800 dark:bg-orange-800 dark:text-orange-100',
                                        ],
                                        'disetujui_pengadaan', 'ditolak_pengadaan' => [
                                            'Pengadaan',
                                        ...
                                        'pengajuan_dikirim_ke_admin', 'processing', 'ready_pickup', 'completed' => [
                                            'Admin',
                                ...
                                @endphp
                                <span
                                    class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium {{ $department[1] }}">
                                    {{ $department[0] }}
                                </span>
                            </div>
                        </div>
