...
class ListMekanikmedia extends ListRecords
{
    ...
    public ?Mekanikfolder $folder = null;
        ...
        return Actions\Action::make('backToParent')
            ...
            ->url(function () {
                if ($this->folder->parent_id) {
                    $parentFolder = Mekanikfolder::find($this->folder->parent_id);
                    if ($parentFolder) {
                        return route('filament.admin.resources.arsip.mekanik.folder.index', [
                            'folder' => $parentFolder->slug // Gunakan slug, bukan folder_id
                        ]);
                    }
                }
                return route('filament.admin.resources.arsip.mekanik.index');
            });
    }
    ...
    protected function backToMainAction()
    {
        return Actions\Action::make('backToMain')
            ...
            ->icon('heroicon-o-arrow-left')
            ->color('gray')
            ->url(route('filament.admin.resources.arsip.mekanik.index'));
    }

    protected function getTableQuery(): Builder
    {
        return Mekanikmedia::query()
            ->where('model_type', Mekanikfolder::class)
    ...
    public function folderAction($folder)
    {
        return Actions\Action::make('openFolder_' . $folder->id)
            ...
            ->action(function () use ($folder) {
                ...
                return redirect()->route('filament.admin.resources.arsip.mekanik.folder.index', [
    ...
    public function createMediaAction()
    {
        return Actions\Action::make('createMedia')
            ...
            ->action(function (array $data) {
                ...
                $folder = Mekanikfolder::find($folder_id);

                foreach ($data['files'] as $file) {
                    $media = new Mekanikmedia();
                    $media->model_type = Mekanikfolder::class;
                    ...
                    $filePath = storage_path('app/public/mekanik/' . $file);
                    $media->mime_type = mime_content_type($filePath);
                    $media->disk = 'mekanik_media';
        ...
        return Actions\Action::make('deleteMedia')
            ...
            ->action(function (array $arguments) {
                try {
                    ...
                    $media = Mekanikmedia::find($arguments['record']['id']);
        ...
        return Actions\Action::make('deleteFolder')
            ...
            ->action(function () {
                try {
                    ...
                    $medias = Mekanikmedia::where('model_type', Mekanikfolder::class)
                    ...
                    if ($parentFolder) {
                        ...
                        return redirect()->route('filament.admin.resources.arsip.mekanik.index');
                    }
                } catch (\Exception $e) {
                    Notification::make()
