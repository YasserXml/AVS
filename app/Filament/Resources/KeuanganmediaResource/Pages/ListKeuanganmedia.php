...
class ListKeuanganmedia extends ListRecords
{
    ...
    public ?Keuanganfolder $folder = null;
    public $subfolders = null;

    public function getTitle(): string|Htmlable
    {
        return $this->folder?->full_name_path ?? 'Media Keuangan';
    }
        ...
        return Actions\Action::make('backToParent')
            ...
            ->url(function () {
                if ($this->folder->parent_id) {
                    $parentFolder = Keuanganfolder::find($this->folder->parent_id);
                    if ($parentFolder) {
                        return route('filament.admin.resources.arsip.keuangan.folder.index', [
                            'folder' => $parentFolder->slug // Gunakan slug, bukan folder_id
                        ]);
                    }
                }
                return route('filament.admin.resources.arsip.keuangan.index');
            });
    }
    ...
    protected function backToMainAction()
    {
        return Actions\Action::make('backToMain')
            ...
            ->icon('heroicon-o-arrow-left')
            ->color('gray')
            ->url(route('filament.admin.resources.arsip.keuangan.index'));
    }
    ...
    public function folderAction($folder)
    {
        return Actions\Action::make('openFolder_' . $folder->id)
            ...
            ->action(function () use ($folder) {
                ...
                return redirect()->route('filament.admin.resources.arsip.keuangan.folder.index', [
    ...
    public function createMediaAction()
    {
        return Actions\Action::make('createMedia')
            ...
            ->action(function (array $data) {
                ...
                $folder = Keuanganfolder::find($folder_id);

                foreach ($data['files'] as $file) {
                    $media = new Keuanganmedia();
                    $media->model_type = Keuanganfolder::class;
                    ...
                    $filePath = storage_path('app/public/keuangan/' . $file);
                    $media->mime_type = mime_content_type($filePath);
                    $media->disk = 'keuangan_media';
        ...
        return Actions\Action::make('deleteMedia')
            ...
            ->action(function (array $arguments) {
                try {
                    ...
                    $media = Keuanganmedia::find($arguments['record']['id']);
        ...
        return Actions\Action::make('deleteFolder')
            ...
            ->action(function () {
                try {
                    ...
                    $medias = Keuanganmedia::where('model_type', Keuanganfolder::class)
                    ...
                    if ($parentFolder) {
                        ...
                        return redirect()->route('filament.admin.resources.arsip.keuangan.index');
                    }
                } catch (\Exception $e) {
                    Notification::make()
