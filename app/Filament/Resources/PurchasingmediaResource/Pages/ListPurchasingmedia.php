...
class ListPurchasingmedia extends ListRecords
{
    ...
    public ?Purchasingfolder $folder = null;
    ...
    public function mount(): void
    {
        ...
        $folder = Purchasingfolder::where('slug', $folderSlug)->first();
        ...
        return Actions\Action::make('backToParent')
            ...
            ->url(function () {
                if ($this->folder->parent_id) {
                    $parentFolder = Purchasingfolder::find($this->folder->parent_id);
                    if ($parentFolder) {
                        return route('filament.admin.resources.arsip.purchasing.folder.index', [
                            'folder' => $parentFolder->slug // Gunakan slug, bukan folder_id
                        ]);
                    }
                }
                return route('filament.admin.resources.arsip.purchasing.index');
            });
    }
    ...
    protected function backToMainAction()
    {
        return Actions\Action::make('backToMain')
            ...
            ->icon('heroicon-o-arrow-left')
            ->color('gray')
            ->url(route('filament.admin.resources.arsip.purchasing.index'));
    }

    protected function getTableQuery(): Builder
    {
        return Purchasingmedia::query()
            ->where('model_type', Purchasingfolder::class)
    ...
    public function folderAction($folder)
    {
        return Actions\Action::make('openFolder_' . $folder->id)
            ...
            ->action(function () use ($folder) {
                ...
                return redirect()->route('filament.admin.resources.arsip.purchasing.folder.index', [
    ...
    public function createMediaAction()
    {
        return Actions\Action::make('createMedia')
            ...
            ->action(function (array $data) {
                ...
                $folder = Purchasingfolder::find($folder_id);

                foreach ($data['files'] as $file) {
                    $media = new Purchasingmedia();
                    $media->model_type = Purchasingfolder::class;
        ...
        return Actions\Action::make('deleteMedia')
            ...
            ->action(function (array $arguments) {
                try {
                    ...
                    $media = Purchasingmedia::find($arguments['record']['id']);
        ...
        return Actions\Action::make('deleteFolder')
            ...
            ->action(function () {
                try {
                    ...
                    $medias = Purchasingmedia::where('model_type', Purchasingfolder::class)
                    ...
                    if ($parentFolder) {
                        ...
                        return redirect()->route('filament.admin.resources.arsip.purchasing.index');
                    }
                } catch (\Exception $e) {
                    Notification::make()
